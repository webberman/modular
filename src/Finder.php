...
namespace Webberman\Modular;

use Exception;
use InvalidArgumentException;
use Illuminate\Support\Collection;
use Webberman\Modular\Entities\Feature;
use Webberman\Modular\Entities\Service;
use Webberman\Modular\Entities\Domain;
use Webberman\Modular\Entities\Job;
...
trait Finder
{
    ...
    /**
     * Determines whether this is a Webberman\Modular microservice installation.
     *
     * @return bool
     */
     ...
     * @throws Exception
     */
    public function findNamespace($dir)
    {
    ...
    {
        return 'Webberman\Modular\Foundation';
    }
    ...
    {
        return 'Webberman\Modular\Units';
    }
    ...
    /**
     * Get the path to a specific livewire component.
     *
     * @param string $service
     ...
     * @return string
     */
    public function findLivewireComponentPath($service, $component)
    {
        $component = str_replace('\\' , DS, $component);
        return $this->findServicePath($service).DS.join(DS, ['Http', 'Livewire', "$component.php"]);
    }

    /**
     * Get the path to the livewire view directory of a specific service.
     *
     * @param string $service
     * @param $view
     *
     * @return string
     */
    public function findLivewireViewPath($service, $view)
    {
        $view = str_replace('.' , DS, $view);
        return $this->findServicePath($service).DS.join(DS, ['resources', 'views', 'livewire', "$view.php"]);
    }

    /**
     * Get the livewire view.
     *
     * @param string $service
     * @param $viewPath
     * @return string
     */
    public function findView($service, $viewPath)
    {
        $base_path = $this->findServicePath($service).DS.join(DS, ['resources', 'views']);

        $view = str_replace( DS, '.', str_replace([$base_path, '.php'], '', $viewPath) );

        return $service . '::' . trim($view, '.');
    }

    /**
     * Get the livewire namespace for the service passed in.
     *
     * @param string $service
     * @param $class
     *
     ...
     * @throws Exception
     */
    public function findLivewireNamespace($service, $class)
    {
        $class = trim($class, '\\') ;

        $namespace  = count( explode('\\', $class) ) > 1 ? '\\' . dirname($class) : '';

        return $this->findServiceNamespace($service).'\\Http\\Livewire' . $namespace ;
    }

...
class Str
{
    ...
    /**
     * Get the given name formatted as a livewire class name.
     *
     * @param string $name
     *
     * @return string
     */
    public static function livewireClass($name)
    {
        if (count($parts = explode('.', $name)) > 1) {
            $className = '';

            foreach ($parts as $part) {
                $className .= self::studly($part) . '\\';
            }
    ...
    /**
     * Get the given name formatted as a livewire view name.
     *
     * @param string $name
     *
     * @return string
     */
    public static function livewireView($name)
    {
        if (count($parts = explode('.', $name)) > 1) {
            $viewName = '';

            foreach ($parts as $part) {
                $viewName .= self::kebab($part) . '.';
            }
